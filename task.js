...
// количество месяцев
      ...
      console.log('Месяцы ' + Math.trunc((Date.parse(date) - Date.now()) / (1000 * 60 * 60 * 24 * 30)));
...
// тело кредита
    ...
    console.log ('Процентная ставка ' + percentageRate);

    //Сумма в месяц
    let totalAmountMonth = S * (percentageRate + (percentageRate / (((1 + percentageRate)** numberOfMonth) - 1)));
    console.log ('Сумма в месяц по формуле ' + totalAmountMonth);
